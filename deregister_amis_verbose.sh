...
while read -r ami_id; do
  ...
  if [ -z "$snapshot_ids" ]; then
    echo "[$(date '+%F %T')] ⚠️ No snapshots found for $ami_id or AMI not found." | tee -a "$log_file"
  else
  ...
  for snapshot_id in $snapshot_ids; do
    echo "[$(date '+%F %T')] 🏷️ Tagging snapshot: $snapshot_id before deletion" | tee -a "$log_file"
    ...
    echo "[$(date '+%F %T')] 🗑️ Deleting snapshot: $snapshot_id" | tee -a "$log_file"
